...
- **The workbook is edited and the build step is never run.** Nothing breaks and the game loads yesterday's numbers — the one failure mode of a build step, and the reason FR-019 exists.
- **The scaffold generator is run against an authored workbook.** It would overwrite real values with template ones. FR-018 requires this be made impossible rather than merely warned against.
...
- **SC-009**: A designer can change a stat and see it in the game **without editing any file other than the workbook**.
- **SC-010**: Content generated from an out-of-date source **cannot reach the game** — continuous integration refuses it.
...
- **`resources/characters/MATCHUPS.md` becomes a generated view once the workbook is canon.** Today it is the roster of record; keeping it hand-maintained alongside an authoritative workbook would put the roster in two places, which Constitution XX rejects. It should be emitted by the same build step that produces the loadable content.
...
## Authoring surface — decided 2026-07-28

> **The spreadsheet is the source of truth. A build step reads it, derives,
> validates, and emits the data the game loads.**

`resources/characters/hero-stats.xlsx` is what a designer edits. It already
carries type dropdowns and a stat-total checker, which is what makes tuning 27
heroes across eight stats bearable — and the **hero-numbers pass is exactly when
that matters most**.

**The arrow reverses.** Today `tools/build-hero-stats.py` *writes* the workbook;
nothing reads it. From here the workbook is read, and two hazards come with that:

### Hazard 1 — the existing generator becomes destructive

`tools/build-hero-stats.py` overwrites the workbook, and its own docstring already
warns: *"Do not run once real stat values are in it."* **A warning in a docstring
is not a safeguard.** The moment the workbook is canon, that script is one
accidental invocation away from erasing the roster.

- **FR-018**: The scaffold generator MUST be made incapable of destroying authored
  content — removed, or gated behind an explicit confirmation that cannot be
  satisfied by running it normally. A comment is not sufficient.

### Hazard 2 — stale generated data, which fails silently

The failure mode of any build step is that someone edits the source and does not
run it. Nothing breaks; the game simply loads yesterday's numbers, and **under the
no-nerf rule a balance pass conducted against stale data is expensive to unwind.**

- **FR-019**: Generated content MUST be committed to the repository, and continuous
  integration MUST regenerate it and **fail if the result differs** from what is
  committed. This is what makes the build step safe rather than merely convenient.
- **FR-020**: The content version (FR-016) MUST derive from the **authored source**,
  so that editing the workbook changes it even if the generated output is
  regenerated identically.

> **Together these mean the spreadsheet's binary opacity costs nothing that
> matters.** A stat change is not reviewable as a diff of the workbook — but it
> *is* reviewable as a diff of the generated output committed beside it, which is
> the artifact the game actually loads.
