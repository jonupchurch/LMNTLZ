...
- [X] **T001** [P] Widen `StatusInstance` in `packages/sim/rules/state.ts` per
  [data-model.md](./data-model.md): typed `kind`, `magnitude`, `stat`,
  `sourcePowerId`, `escalation`, `cleansable`. Keep `turnsRemaining` and
  `sourceInstanceId`.
- [X] **T002** Create `packages/sim/rules/status.ts` with `STATUS_CATALOG` — the
  11 kinds across 6 families, each declaring its `StackRule` and whether it
  ticks damage.
- [X] **T003** [P] Add the tier-derived magnitude functions to `status.ts`:
...
- [X] **T004** Test `packages/sim/tests/rules/status.test.ts`: assert each tier
...
- [X] **T005** Update every `HeroState` fixture that constructs `statuses: []`
...
- [X] **T006** Implement `applyStatus(existing, incoming)` in `status.ts`,
...
- [X] **T007** [P] Implement `tickDurations` and `cleanse` in `status.ts`.
  `cleansable: false` survives a cleanse and still expires.
- [X] **T008** Test the six stacking rows from
...
- [X] **T009** Add `statusPoints(hero, key)` to `status.ts` and make
  `effectiveStat` in `state.ts` sum it **on top of** `hero.statMods`.
- [X] **T010** 🔴 Test [quickstart.md](./quickstart.md) scenario 4: a hero with a
...
- [X] **T011** [P] Add `shredFactor`, `shieldOf`, `upkeepDamage` to `status.ts`.
- [X] **T012** Read the shred layer in `packages/sim/rules/damage.ts` where `E`
...
- [X] **T013** Deplete `shieldOf` before the health pool, passing the remainder
  of a breaking hit through in the same step.

### The rider data

- [X] **T014** Add `riders` to `powerSchema` in `packages/content/src/schema.ts`
...
- [X] **T015** Create `tools/power-riders.json`. **Author all 87 active powers**
  ...
  prose.
- [X] **T016** Read and drift-check the file in `tools/build-content.ts`, failing
  the build when it names a power the workbook does not have — same check
  `power-targeting.json` already uses.
- [X] **T017** Test `packages/content/tests/riders.test.ts`: every active power
  appears exactly once; no rider carries a magnitude; every `buff`/`debuff`/
  `shred` names a stat.

### Resolution

- [X] **T018** Populate **step 3** of `resolveOne` in
...
- [X] **T019** ⭐ **WIRING** — `apps/api/src/battle/turnLoop.ts` `takeTurn`
  ...
  exercised.
- [X] **T020** ⭐ **WIRING** — `resolveOne` passes the statuses' taunt and fade
...
- [X] **T021** Apply the `Toughness`-buff rule in `turnLoop.ts`: raise max HP
  **and** grant the same amount as current HP; on expiry lower max HP and clamp
  current down to it.
- [X] **T022** Bump `engineVersion()` in `packages/sim/rules/index.ts` to
  `e0.3.0`. **Constitution XVI.**
- [X] **T023** 🔴 Verify the four determinism suites — `drawOrder`,
  `seedCustody`, `determinism`, `reDerive` — are green, and add a case asserting
  a **friendly** power consumes no rider draw.
- [X] **T024** 🔴 Test [quickstart.md](./quickstart.md) scenario 7: a replay
  recorded pre-020 plays back byte-identically and reports the *old* engine
  version.
- [X] **T025** Document the drain-before-switch step in the deploy notes. An
...
- [X] **T042** Carry statuses on the action packet in
...
- [X] **T043** 🔴 Test the **payload**, not the render: an enemy self-effect's
...
- [X] **T044** [P] Add the status icon mapping to
  ...
  (Constitution XIII).
- [X] **T045** Build `StatusRow.tsx`: a pip per effect, a numeral only where the
  rule permits.
- [X] **T046** Reserve the height of the part that **varies**, and never clear on
  `mouseleave`. *A panel that resizes as effects come and go shakes everything
  below it.*
- [X] **T047** [P] Unit-test the three visibility classes.
- [X] **T048** Playwright: a real browser sees the pips, because jsdom does no
  layout.
- [X] **T049** Show a burning champion's remaining ticks in the battle log line,
  so the transcript and the board agree.
- [X] **T050** ⭐ **WIRING** — `BattleScreen` renders `StatusRow` for every hero
