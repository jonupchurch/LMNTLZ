    ...
    return (
      <div className="rounded-lg border border-emerald-800/60 bg-emerald-950/20 p-4 text-body">
        ...
        <p className="mt-1 text-stone-300">
          ...
          <strong className="text-stone-100">That has already been cancelled</strong>{' '}
          — signing in was all it took, and it is why we never put a link in that
          email. Nothing else is needed from you.
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="rounded-lg border border-amber-800/60 bg-amber-950/20 p-4 text-body">
        ...
        <p className="mt-1 text-stone-300">
          ...
          <strong className="text-stone-100">{completes.toLocaleString()}</strong> unless
    ...
    try {
      ...
      setError(
        err instanceof ApiError && err.status === 402
          ? 'Inheriting costs 650 shards, and you need them at the end as well as now.'
  ...
  return (
    <details className="rounded-lg border border-stone-800 p-4 text-body">
      ...
      <p className="mt-2 text-stone-400">
        If the master has not played for 14 days you can ask to take over. They then
        get 7 days, and{' '}
        <strong className="text-stone-200">signing in once cancels it</strong> — so
        this is slow on purpose. If it completes you pay 650 shards and the former
        master is refunded 650 and stays on as a member.
      </p>
      <button
        type="button"
        disabled={busy}
        className="mt-3 rounded border border-stone-700 px-3 py-1 disabled:opacity-40"
