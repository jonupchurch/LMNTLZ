...
- **A guild emblem is composed from parts we author**, never uploaded: **one of 36 icons, an icon colour from a vivid palette of 12, and a background colour from a separate dark palette of 12** — **5,184 combinations**, every one legible because the two palettes never overlap. Guilds get real expression and nothing needs moderating. **One of the 36 is blank**, so a guild wanting a plain field of colour picks it like any other icon — no toggle, no special case. **A contrast readout may warn but must never gate**: it cannot disable the save or mark the emblem invalid. Harm is a gate; taste is a note.
