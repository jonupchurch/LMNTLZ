...
- `designsystem/` — the rendered Claude Design deliverables. **Anything landing in this folder is intentional and gets committed.**
  ...
  - `LMNTLZ Guild Admin.dc.html` — GM/Officer management: invite, kick, recruitment mode, tagline, and Wing assignment. Enforces `CAP = 8` correctly, and when a target Wing is full it demands a **swap** rather than refusing the move — which quietly answers *who assigns members to Wings* (officers, at any time). **One gap:** assignments now **lock when an event starts**, and the screen has no locked state. During an event it needs a countdown to the lock, Wing reassignment and invites disabled, and **remove-from-guild left enabled** — that one action stays available, and the vacated seat must read as unfillable rather than empty. See `mechanics/08-guilds.md`.
