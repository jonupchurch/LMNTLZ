...
9. **Commit often, atomically; branch per feature.** Work on a feature branch,
   ...
   and verified.
10. **Ask one question at a time.** One question, then wait for the answer
    before the next. Use an interactive prompt with selectable options wherever
    the tooling supports it, rather than burying the question in prose. Never
    batch several decisions into one multi-part prompt — the first answer
    usually changes what the second question should be. This doesn't license
    asking *more* questions; rule 1's bar for what merits asking is unchanged.
11. **Lead long summaries with a layman's TL;DR.** Any long write-up — review,
    design rationale, plan readback, completion report — opens with a short
    plain-language summary for someone without the context loaded: no jargon,
    no acronyms, what it means and why it matters rather than the mechanics.
    The full detail still follows; the TL;DR is an addition, not a replacement.
...
- Stacking several questions into one prompt because it feels more efficient.
- Ending a long technical write-up without a plain-language summary on top.
